# Example 1 AJEL - Normal Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
for (ii in 1:length(rho)) {
  ...
  for (jj in 1:iter) {
    ...
    U = rnorm(n, mean = 2, sd = 1)  # Normal distribution for U
    
    ######## DISTORTING FUNCTION #################
    
    phi_X = (U - 2)^2 - 1  # E[(U-2)^2] = 1, so E[phi_X] = 0
    phi_Y = cos(U) + 0.2522  # E[cos(U)] ≈ -0.2522, so E[phi_Y] = 0
...
# Example 1 EL - Normal Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
for (ii in 1:length(rho)) {
  ...
  for (jj in 1:iter) {
    ...
    e.centered = (e.X.est - mean(e.X.est)) * (e.Y.est - mean(e.Y.est))
    il.eltest = el.test(e.centered, 0)
    il.el = il.eltest$'-2LLR'
    
    ci.el = findci(e.centered)
...
# Example 1 JEL - Normal Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
# Example 1 MJEL - Normal Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
# Example 1 TJEL - Normal Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
for (ii in 1:length(rho)) {
  ...
  for (jj in 1:iter) {
    ...
    # Avoiding values outside (-1, 1) due to arctanh
    V[V <= -0.9999] <- -0.9999
    V[V >=  0.9999] <-  0.9999
