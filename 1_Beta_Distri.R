# Example 1 AJEL - Beta Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
for (ii in 1:length(rho)) {
  ...
  for (jj in 1:iter) {
    ...
    U = rbeta(n, shape1 = 2, shape2 = 8)  # Beta distribution for U
    
    ######## DISTORTING FUNCTION #################
    
    phi_X = 0.31365 - 3 * (U - 0.5)^2  # Adjusted to have E[phi_X] = 0
    phi_Y = 0.4045 + 0.5 * cos(2 * pi * U)  # Adjusted to have E[phi_Y] = 0
...
# Example 1 EL - Beta Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
for (ii in 1:length(rho)) {
  ...
  for (jj in 1:iter) {
    ...
    phi_X = 0.31365 - 3 * (U - 0.5)^2       # Centered distortion function
    phi_Y = 0.4045 + 0.5 * cos(2 * pi * U)  # Centered cosine distortion
    ...
    estimating.eq = (e.X.est - mean(e.X.est)) * (e.Y.est - mean(e.Y.est))
...
# Example 1 JEL - Beta Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
for (ii in 1:length(rho)) {
  ...
  for (jj in 1:iter) {
    ...
    U = rbeta(n, shape1 = 2, shape2 = 8)
    
    ######## DISTORTION FUNCTION #################
    phi_X = 0.31365 - 3 * (U - 0.5)^2
    phi_Y = 0.4045 + 0.5 * cos(2 * pi * U)
...
# Example 1 MJEL - Beta Distribution for U
library(MASS)
library(emplik)
library(kedd)
...
# Example 1 TJEL - Beta Distribution for U
library(MASS)
library(emplik)
library(kedd)
