# Install the required packages (Uncomment if not already installed)
#install.packages('emplik')
#install.packages('kedd')

library(MASS)
library(emplik)
library(kedd)

######### PARAMETERS ########################
n = 50
rho = c(-0.9, -0.5, 0, 0.5, 0.9)
iter = 100

######## Define 3D Array for Results ########
results = array(NA, dim = c(5, 4, 5), dimnames = list(
  paste("Rho =", rho),
  c("Lower CI", "Upper CI", "CI Length", "Coverage"),
  c("EL", "JEL", "AJEL", "MJEL", "TJEL")
))

########## FUNCTIONS #########################
get.NWK = function(x,u,small.u){
  bw = density(u, kernel = c("epanechnikov"))$bw
  KX = vector()
  K = vector()
  for(j in 1:length(u)){
    d = (u[j]-small.u)/bw
    KX[j] = exp(x[j])*(bw^(-1)*max(0,0.75*(1-d^2)))
    K[j] = (bw^(-1)*max(0,0.75*(1-d^2)))
  }
  result = sum(KX)/sum(K)
  return(result)
}

findci <- function(x.vector, method = "EL") {
  cifunc <- function(ci.val) {
    x.vector = x.vector - ci.val
    if (method == "AJEL") {
      x.vector[length(x.vector) + 1] = -0.5 * log(length(x.vector)) * mean(x.vector)
    } else if (method == "MJEL") {
      x.vector = x.vector / sqrt(var(x.vector))
    } else if (method == "TJEL") {
      x.vector = log(x.vector - min(x.vector) + 1)
    }
    el.test(x.vector, 0)
  }
  
  vectorMean <- mean(x.vector)
  
  tryCatch({
    L = findUL2(fun = cifunc, MLE = vectorMean, x.vector = x.vector)$Low
    U = findUL2(fun = cifunc, MLE = vectorMean, x.vector = x.vector)$Up
  }, error = function(e) {
    L <<- vectorMean - 0.1  # Fallback for lower bound
    U <<- vectorMean + 0.1  # Fallback for upper bound
  })
  
  return(list(Up = U, Low = L))
}

############ ITERATIONS #######################
for (ii in 1:5) {
  for (method in c("EL", "JEL", "AJEL", "MJEL", "TJEL")) {
    coverage.jel = vector()
    length.jel = vector()
    upper.jel = vector()
    lower.jel = vector()
    
    for (jj in 1:iter) {
      
      ####### DATA GENERATION #####################
      sigma <- matrix(c(1, rho[ii], rho[ii], 1), nrow = 2)
      data <- mvrnorm(n, mu = c(0,0), Sigma = sigma) 
      colnames(data) <- c("X","Y")
      
      U = runif(n,-1,1)
      
      ######## DISTORTING FUNCTION ################
      phi_X = log(1+0.75*sin(2*pi*U))
      phi_Y = -U + log(2/(exp(1)-exp(-1)))
      
      ###### OBSERVED DATA ########################
      xy.obs <- data + cbind (phi_X,phi_Y)
      
      ####### CALCULATING ESTIMATORS ##############
      e.Y.est = vector()
      e.X.est = vector()
      
      for (i in 1:n){
        e.Y.est[i] = xy.obs[,2][i] - log(get.NWK(xy.obs[,2],U,U[i])) + log(mean(exp(xy.obs[,2])))
        e.X.est[i] = xy.obs[,1][i] - log(get.NWK(xy.obs[,1],U,U[i])) + log(mean(exp(xy.obs[,1])))
      }
      
      cov.e.est = mean(e.X.est*e.Y.est) - mean(e.X.est)*mean(e.Y.est)
      sig.e.X = mean(e.X.est*e.X.est) - mean(e.X.est)^2
      sig.e.Y = mean(e.Y.est*e.Y.est) - mean(e.Y.est)^2
      rho.e.est = (cov.e.est)/(sqrt(sig.e.X*sig.e.Y))
      
      ############ JACKKNIFING #######################
      rho.j=vector()
      for (j in 1:n){
        xy.jack=xy.obs[-j,]
        U.jack = U[-j]
        
        Eix.j = vector()
        Eiy.j = vector()
        
        for (k in 1:(n-1)){
          Eix.j[k] = xy.jack[,1][k]-log(get.NWK(xy.jack[,1],U.jack,U.jack[k]))+log(mean(exp(xy.jack[,1])))
          Eiy.j[k] =xy.jack[,2][k]-log(get.NWK(xy.jack[,2],U.jack,U.jack[k]))+log(mean(exp(xy.jack[,2])))
        }
        
        cov.e.est.jack = mean(Eix.j*Eiy.j) - mean(Eix.j)*mean(Eiy.j)
        sig.e.X.jack = mean(Eix.j*Eix.j) - mean(Eix.j)^2
        sig.e.Y.jack = mean(Eiy.j*Eiy.j) - mean(Eiy.j)^2
        rho.e.est.jack = (cov.e.est.jack)/(sqrt(sig.e.X.jack*sig.e.Y.jack))
        
        rho.j[j]=n*rho.e.est-(n-1)*rho.e.est.jack
      }
      
      ######### EMPIRICAL LIKELIHOOD #################
      ci.jel = findci(rho.j, method = method)
      upper.jel[jj] = ci.jel$Up
      lower.jel[jj] = ci.jel$Low
      length.jel[jj] = upper.jel[jj] - lower.jel[jj]
      coverage.jel[jj] = (ci.jel$Low <= rho[ii] & ci.jel$Up >= rho[ii])
    }
    
    results[ii, , method] = c(mean(lower.jel), mean(upper.jel), mean(length.jel), mean(coverage.jel))
  }
}

print(results)
