...
get.NWK = function(x, u, small.u) {
  ...
  for (j in 1:length(u)) {
    d = (u[j] - small.u) / bw
    KX[j] = x[j] * (bw^(-1) * max(0, 0.75 * (1 - d^2)))  # Changed to use x directly (no exp)
...
for (ii in 1:5) {
  for (jj in 1:iter) {
    ...
    # For multiplicative distortion, ensure the functions are positive
    psi_X = 1 + 0.75 * sin(2 * pi * U)  # Positive function (ranges from 0.25 to 1.75)
    psi_Y = 1 + 0.5 * U  # Positive function (ranges from 0.5 to 1.5)
    
    ###### OBSERVED DATA #########################
    
    # Multiplicative distortion: X * psi_X, Y * psi_Y
    xy.obs <- data * cbind(psi_X, psi_Y)
    
    # Work in log scale for residual estimation
    log.xy.obs <- log(xy.obs)
    ...
    e.Y.est = vector()
    for (i in 1:n) {
      e.Y.est[i] = log.xy.obs[, 2][i] - get.NWK(log.xy.obs[, 2], U, U[i]) + mean(log.xy.obs[, 2])
    }
    
    e.X.est = vector()
    for (i in 1:n) {
      e.X.est[i] = log.xy.obs[, 1][i] - get.NWK(log.xy.obs[, 1], U, U[i]) + mean(log.xy.obs[, 1])
    }
    ...
    for (j in 1:n) {
      xy.jack = xy.obs[-j, ]
      U.jack = U[-j]
      log.xy.jack = log(xy.jack)
      
      Eix.j = vector()
      Eiy.j = vector()
      
      for (k in 1:(n - 1)) {
        Eix.j[k] = log.xy.jack[, 1][k] - get.NWK(log.xy.jack[, 1], U.jack, U.jack[k]) + mean(log.xy.jack[, 1])
        Eiy.j[k] = log.xy.jack[, 2][k] - get.NWK(log.xy.jack[, 2], U.jack, U.jack[k]) + mean(log.xy.jack[, 2])
      }
