...
get.NWK = function(x,u,small.u){
  bw = sd(u) * length(u)^(-1/3)  # Silverman bandwidth
  KX = vector()
  K = vector()
  for(j in 1:length(u)){
    d = (u[j]-small.u)/bw
...
for (ii in 1:5){
  ...
  for (jj in 1:iter){
    ...
    U = runif(n, -1, 1)  # or try U ~ Beta(2,2), etc.
    
    ######## MULTIPLICATIVE DISTORTION ##########
    
    # Ensure E[ψ(U)] = E[φ(U)] ≈ 1 (centered functions)
    psi_U = exp(0.5 * sin(pi * U))  # E ≈ 1
    phi_U = exp(0.5 * cos(pi * U))  # E ≈ 1
